...
/** Compact inline timeline shown in chat after task completion */
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
      <button
        ...
        className={cn(
          "w-full flex items-center gap-2 rounded-xl px-3 py-2 text-left transition-all duration-300",
      ...
      {expanded && (
        <div className="mt-1.5 ml-2 pl-3 border-l border-border/50 space-y-1 animate-in fade-in slide-in-from-top-1 duration-300">
          ...
          {task.url && (
            <a
              ...
              className="inline-flex items-center gap-1 text-[10px] font-medium text-primary hover:text-primary/80 transition-colors pt-1"
            >
              Open result →
            </a>
          )}
  ...
  return (
    <div>
      ...
      {showOutput && action.output && (
        <pre className="text-[9px] font-mono bg-background/80 rounded p-1.5 mt-0.5 ml-3 overflow-x-auto text-muted-foreground border border-border/30 max-h-16 overflow-y-auto animate-in fade-in duration-200">
          {action.output}
        </pre>
      )}
    </div>
